...
PAPER_EPS = {
    "fig1": "figure-1.eps", "fig2": "figure-3.eps", "fig3": "figure-4.eps",
    "fig4": "figure-5.eps", "fig5": "figure-2.eps",
}
...
def reproduce_comparison(seed, cfg) -> None:
    ...
    n_exp = cfg["cmp_n_exp"]
    print(f"[Fig 3&4] method comparison: {len(mu_true)} mu points, "
          f"n_exp={n_exp}/point/panel, engine grid {cfg['cmp_grid'][0]:.1f}.."
          f"{cfg['cmp_grid'][1]:.0f} x{cfg['cmp_grid'][2]} (n_cal={cfg['cmp_ncal']})")
    engine = ComparisonEngine(
        np.geomspace(*cfg["cmp_grid"]), n_cal=cfg["cmp_ncal"],
        rng=np.random.default_rng(seed),
    )
    print("  engine built; running experiments...")

    ratios = {"a": {m: [] for m in _METHOD_STYLE},
              "b": {m: [] for m in _METHOD_STYLE}}
    ...
    panel_titles = {"a": "(a) no background",
    ...
    plt.close(fig)
    print(f"  wrote {out.relative_to(REPO)}")
    side_by_side(out, paper_png("fig3"), FIGS / "fig03_side_by_side.png",
    ...
    plt.close(fig)
    print(f"  wrote {out.relative_to(REPO)}")
    side_by_side(out, paper_png("fig4"), FIGS / "fig04_side_by_side.png",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--only", choices=["fig2", "c0", "fig5", "compare", "explain"],
    ...
    if full:
        cfg = dict(
            fig2_n=50000, fig2_mu=45, c0_n=200000, fig5_n=40000, fig5_mu=30,
            cmp_grid=(2.303, 110.0, 48), cmp_ncal=40000,
            cmp_mu_true=np.linspace(2.5, 35.0, 24), cmp_n_exp=20000,
        )
    else:
        cfg = dict(
            fig2_n=8000, fig2_mu=25, c0_n=60000, fig5_n=8000, fig5_mu=18,
            cmp_grid=(2.303, 45.0, 22), cmp_ncal=6000,
            cmp_mu_true=np.linspace(2.5, 16.0, 9), cmp_n_exp=1500,
        )
